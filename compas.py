...
df = df.fillna(-1)
dropFeats = ["decile_score", "id", "name", "first", "last", "c_case_number", "r_case_number", "vr_case_number", "v_type_of_assessment", "type_of_assessment", "screening_date", "v_screening_date", "score_text"]
df = df.drop(dropFeats, axis=1)
df['compas_screening_date'] = df['compas_screening_date'].astype('datetime64').astype(int)/100000000000
...
df['c_jail_out'] = df['c_jail_out'].astype('datetime64').astype(int)/100000000000
df['c_offense_date'] = df['c_offense_date'].astype('datetime64').astype(int)/100000000000
df['c_arrest_date'] = df['c_arrest_date'].astype('datetime64').astype(int)/100000000000
df['r_offense_date'] = df['r_offense_date'].astype('datetime64').astype(int)/100000000000
df['r_jail_in'] = df['r_jail_in'].astype('datetime64').astype(int)/100000000000
df['r_jail_out'] = df['r_jail_out'].astype('datetime64').astype(int)/100000000000
df['vr_offense_date'] = df['vr_offense_date'].astype('datetime64').astype(int)/100000000000
...
x = cleanX.values #returns a numpy array
...
trainloader = DataLoader( dataset = compas , batch_size= 64 , shuffle = True)
...
X_train_bias, X_test_bias, y_train, y_test = train_test_split(X, y, test_size=0.33, random_state=42)
...
forest = RandomForestClassifier(n_estimators=3, random_state=0, max_depth=5, n_jobs=1)
...
del forest
del predict
forestObscured = RandomForestClassifier(n_estimators=5, random_state=0, max_depth=5)
